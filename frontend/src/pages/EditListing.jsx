...
function EditListing () {
  ...
  useEffect(() => {
    apiRequest(`/listings/${id}`, 'GET', {}, false)
      .then((response) => {
        setFormData(response.listing)
        const metaData = response.listing.metadata
        setBedroomTypes(metaData.bedroomTypes ? metaData.bedroomTypes : [])
        setPropertyAmenities(metaData.propertyAmenities ? metaData.propertyAmenities : [])
        setPropertyImages(metaData.propertyImages ? metaData.propertyImages : [])
        setPropertyType(metaData.propertyType ? metaData.propertyType : '')
        setNumBathrooms(metaData.numBathrooms ? metaData.numBathrooms : 0)
      })
      .catch((error) => {
  ...
  const handleUpdateListing = () => {
    ...
    apiRequest(`/listings/${id}`, 'PUT', uploadData, false)
      .then((response) => {
      ...
      <form>
        ...
        <Box mt={ 2 }>
          ...
          <Button startIcon={ <AddIcon /> } variant="outlined" component="label">
            Upload
            <input type="file" accept="image/*" onChange={ handelpropertyImagesUpload } style={ { display: 'none' } } />
          </Button>
        ...
        <Box mt={ 4 }>
          <Button id="edit-listing-btn" variant="contained" color="primary" onClick={ handleUpdateListing }>
            Update Listing
          </Button>
        </Box>
